...
const Work = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = Background.on("change", updateBackgroundState);
    return () => {
      unsubscribe();
    };
  }, [Background]);
  ...
  /* ANIMATIONS */
  const scale = useTransform(WorkTitle, [0, 0.5], [1.5, 1]);
  // const translateY = useTransform(WorkTitle, [0.5, 1], ["100vh", "0vh"]);
  ...
  const translateTitles = useTransform(Background, [0.9, 1], ["0vh", "-350px"]);

  const opacity = useTransform(WorkTitle, [0.5, 1], [1, 0.4]);
  ...
  /* 0.5 -> 200vh */
  const randomImageOpacity = useTransform(Works, [0.4, 0.5], [1, 0]);
  const rotateX1 = useTransform(Works, [0.1, 0.375], ["50deg", "-50deg"]);
  const rotateX2 = useTransform(Works, [0.2, 0.535], ["50deg", "-50deg"]);
  const rotateX3 = useTransform(Works, [0.35, 0.635], ["50deg", "-50deg"]);
  ...
  return (
    <div
      ref={ref}
      className={`h-[200dvh] sm:h-[380dvh] ${
        backgroundOn ? "bg-white" : "bg-transparent"
      }`}
    >
      <section className="work-section-container">
        ...
        <motion.div
          style={{ translateY }}
          className="absolute top-0 w-full min-h-[100vh] bg-[#222] gray-background"
        ></motion.div>

        {/* <motion.div
          style={{ translateY }}
          className="sm:hidden absolute top-0 w-full min-h-[100vh] bg-[#222] "
        ></motion.div> */}
      </section>
      <MobileWorks />
