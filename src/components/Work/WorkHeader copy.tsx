...
const WorkHeader = () => {
  ...
  const { scrollYProgress: WorkTitle } = useScroll({
    target: ref,
    offset: ["-100vh start", "200vh end"],
  });

  const { scrollYProgress: Works } = useScroll({
    target: ref,
    offset: ["start start", "600vh end"],
  });
  ...
  const scale = useTransform(
    ...
    [0, 0.5],
    [1.5, 1]
  ); /* Scale from 0vh to 100vh */
  const translateY = useTransform(
    WorkTitle,
    [0.5, 1],
    ["100vh", "0vh"]
  ); /* Translate from 100vh to 200vh */
  const opacity = useTransform(
    WorkTitle,
    [0.5, 1],
    [1, 0.4]
  ); /* Opacity from 100vh to 200vh */
  const filter = useTransform(WorkTitle, (v) => {
    if (v < 0.7) return `blur(0px)`;
    const blurValue = (v - 0.7) * (10 / 0.35);
    return `blur(${blurValue}px)`;
  }); /* Filter from 100vh to 200vh */
  ...
  const y = useTransform(Works, [0.2, 1], [0, -viewportHeight * 4]);

  // const keyImgRef1 = useRef(null);
  // const keyImgRef2 = useRef(null);
  // const keyImgRef3 = useRef(null);

  // const isInView1 = useInView(keyImgRef1, { margin: "0%", amount: 0.5 });
  // const isInView2 = useInView(keyImgRef2, { margin: "0%", amount: 0.45 });
  // const isInView3 = useInView(keyImgRef3, { margin: "0%", amount: 0.45 });

  const controls = useAnimation();
  const { scrollY } = useScroll();
  // const scrollTrigger1 = 2800; // In px
  // const scrollTrigger2 = 3400; // In px
  // const scrollTrigger1 = viewportHeight * 3.15;
  // const scrollTrigger2 = viewportHeight * 3.7;
  ...
  useEffect(() => {
    const updateScrollTriggers = () => {
      if (imgRef1.current && imgRef2.current) {
        ...
        // Set scrollTrigger values based on image positions
        setScrollTrigger1(img1Rect.top + window.scrollY);
        setScrollTrigger2(img2Rect.top + window.scrollY);
      }
    };

    // Update scroll triggers on resize and initial load
  ...
  useEffect(() => {
    ...
    return scrollY.onChange(handleScroll); // Check on scroll
  }, [scrollY, controls, scrollTrigger1, scrollTrigger2]);

  return (
    <div ref={ref} className="h-[380dvh]">
      <section className="work-section-container">
        ...
        <motion.div
          style={{ translateY }}
          className="absolute top-0 w-full min-h-[100vh] bg-[#222]"
        ></motion.div>
      </section>
      <section className="works-container sticky top-0 text-white max-h-[100dvh]">
        <motion.div style={{ y }} className="works-container">
          <a href="" className="work-item-link">
            <div className="persp">
              <motion.img
                ...
                src="galaxy.jpg"
                alt="Miracle UI"
                className="work-item-image rounded-3xl"
              />
            </div>
          </a>
          <a href="" className="work-item-link">
        ...
        <div className="work-titles-outercontainer">
